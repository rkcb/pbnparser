...
public class JsonEventsTest {
	...
	@Test
	public void OptimumScoreTableTest() {
		events = new JsonEvents(ToolsTest.rawEvents("test2"));
		P.on(events.get(0).getOptimumResultTable().getHeader().toString());
		JsonOptimumResultTable t = events.get(0).getOptimumResultTable();
		JsonOptimumResultTable.sortRows(t);
